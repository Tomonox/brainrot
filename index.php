...
foreach ($stats_hebdo as $sem => $data) {
    $labels_hebdo[] = "Sem. " . substr($sem, 5);
    $data_hebdo[] = $data['usd'];
}

function getCumulArray($transactions, $target_month_key, $limit_day = 31) {
    $daily_sum = array_fill(1, 31, 0);
    if (isset($transactions[$target_month_key])) {
        foreach ($transactions[$target_month_key] as $t) $daily_sum[(int)date('d', strtotime($t['date_vente']))] += $t['montant'];
    }
    $cumul = []; $running = 0;
    for ($i=1; $i<=31; $i++) {
        if ($i > $limit_day) break;
        $running += $daily_sum[$i];
        $cumul[] = $running;
    }
    return $cumul;
...
<head>
    ...
    <style>
        ...
        .kpi-value { font-size: 2.1rem; font-weight: 700; color: #4a6fa5; position: relative; z-index: 2; }
        ...
        .history-title { text-align: center; color: #4a6fa5; font-weight: 600; font-size: 1.6rem; margin-bottom: 1rem; position: relative; }
        ...
        .monthly-tab-btn { background: none; border: none; padding: 8px 15px; margin: 5px; border-radius: 20px; font-weight: 500; color: #4a6fa5; cursor: pointer; transition: background-color 0.2s, color 0.2s; }
        .monthly-tab-btn:hover { background-color: #d1e0f6; }
...
<div class="container">
    ...
    <div class="row g-3 mb-5 justify-content-center">
        <div class="col-md-3 col-6">
            <div class="badge-stat">
                ...
                <div><span class="val"><?= number_format($max_win, 2, ',', ' ') ?> $</span><span class="lbl">Record Retrait</span></div>
            </div>
        </div>
        <div class="col-md-3 col-6">
            <div class="badge-stat">
                <i class="fas fa-chart-line text-info"></i>
                <div><span class="val"><?= number_format($avg_win, 2, ',', ' ') ?> $</span><span class="lbl">Moyenne / Retrait</span></div>
            </div>
        </div>
        <div class="col-md-3 col-6">
            <div class="badge-stat">
                <i class="fas fa-bullseye text-danger"></i>
                <div><span class="val"><?= number_format($projection_usd, 2, ',', ' ') ?> $</span><span class="lbl">Projection Fin Mois</span></div>
            </div>
        </div>
    </div>
    ...
    <div class="row g-4 mb-5">
        <div class="col-md-3">
            <div class="card p-3 text-center"> 
                ...
                <div class="text-muted small">💸 CA USD du Mois Actuel</div>
        ...
        <div class="col-md-3">
            <div class="card p-3 text-center">
                <div class="text-muted small">💰 CA EUR Réel (Indicatif)</div>
        ...
        <div class="col-md-3">
            <div class="card p-3 text-center">
                <div class="text-muted small">📊 Évolution USD (Jours 1-<?= (int)$jour_actuel ?>)</div>
                <div class="evolution-kpi <?= $color_evol ?>">
                    <?= $symbole_evol ?> <?= number_format(abs($evolution_pct), 1, ',', ' ') ?> %
                </div>
                <div class="text-muted small" style="line-height: 1.2;">
                    <?php echo 'vs ' . traduire_mois(date('F', strtotime('last month'))); ?>
                    <br>(<?= number_format($ca_precedent_pmtd_usd, 2, ',', ' ') ?> $)
                </div>
            </div>
        </div>
    ...
    <div class="history-card mb-5">
        ...
        <div class="monthly-tabs-nav" id="monthly-tabs">
            <?php $is_first = true; foreach ($retraits_par_mois as $mois_key => $retraits): 
                $active_class = $is_first ? 'active' : ''; ?>
                <button class="monthly-tab-btn <?= $active_class ?>" data-target="<?= $mois_key ?>">
                    <?= traduire_mois(date('F Y', strtotime($mois_key . '-01'))) ?>
                </button>
                    ...
                    <table class="table table-hover table-striped detail-table table-custom">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td><span class="rate-badge">1$ = <?= $retrait['taux_appliq'] ?>€</span></td>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    new Chart(ctxCumul, {
        ...
        data: {
            ...
            datasets: [
                {
                    label: 'Ce Mois-ci (Cumul USD)',
                    data: <?= json_encode($data_cumul_actuel) ?>,
                    borderColor: '#4a6fa5', backgroundColor: 'rgba(74, 111, 165, 0.1)',
                    borderWidth: 3, pointRadius: 3, fill: true, tension: 0.3
                },
                {
                    label: 'Mois Dernier (Cumul USD)',
                    data: <?= json_encode($data_cumul_prev) ?>,
                    borderColor: '#95a5a6', backgroundColor: 'transparent',
                    borderDash: [5, 5], borderWidth: 2, pointRadius: 0, fill: false, tension: 0.3
                }
            ]
        },
        options: {
            ...
            plugins: { legend: { position: 'top' }, tooltip: { callbacks: { label: c => c.dataset.label + ': ' + new Intl.NumberFormat('fr-FR',{style:'currency',currency:'USD'}).format(c.parsed.y) } } },
    ...
    new Chart(weeklyCtx, {
        type: 'bar',
        data: {
            labels: <?= json_encode($labels_hebdo) ?>,
            datasets: [{
                label: 'CA Semaine (USD $)',
                data: <?= json_encode($data_hebdo) ?>,
                backgroundColor: 'rgba(74, 111, 165, 0.7)', borderColor: 'rgba(74, 111, 165, 1)', borderWidth: 1
            }]
        },
        options: {
            responsive: true, maintainAspectRatio: false,
            scales: { y: { beginAtZero: true }, x: { } },
            plugins: { legend: { display: false }, tooltip: { callbacks: { label: c => new Intl.NumberFormat('fr-FR',{style:'currency',currency:'USD'}).format(c.parsed.y) } } }
        }
    });

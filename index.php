...
<head>
    ...
    <style>
        ...
        /* ======================================================
           📱 OPTIMISATIONS MOBILE AMÉLIORÉES
           ====================================================== */
        @media (max-width: 768px) {
            body { padding-top: 15px; } /* Moins d'espace en haut pour pas gaspiller */

            /* Header qui ne "part plus en couille" */
            .header-container {
            ...
            /* Onglets scrollables proprement */
            .monthly-tabs-nav {
                flex-wrap: nowrap;
            ...
            .monthly-tabs-nav::-webkit-scrollbar { display: none; /* Chrome/Safari */ }
            .monthly-tab-btn {
                white-space: nowrap;
                flex: 0 0 auto;
                font-size: 0.9rem;
            ...
            .card {
                margin-bottom: 10px;
                padding: 1rem !important; /* Padding interne réduit */
            }
            .kpi-value, .evolution-kpi { 
                font-size: 1.6rem; /* Police un peu plus petite */
            }
            
            /* Login overlay compact */
            .login-card { 
                width: 95%; 
                padding: 1.5rem 1rem; /* Moins de padding */
                margin-top: -50px; /* Remonter un peu visuellement */
            }

            /* Stats Top Row (Record/Moyenne/Proj) */
            .badge-stat {
                padding: 8px 5px; /* Très compact */
                gap: 5px;
                justify-content: center;
                flex-direction: column; /* Icone au dessus du chiffre */
                text-align: center;
                height: 100%;
            }
            .badge-stat i { margin-bottom: 2px; }
            .badge-stat .val { font-size: 0.9rem; font-weight: 700; }
            
            .chart-container {
                height: 250px; /* Graphique moins haut sur mobile */
            }
        }
    </style>
</head>
...
<div class="login-overlay">
    <div class="login-card">
        ...
        <p class="text-muted small mb-3">Veuillez vous identifier.</p>
        <form method="POST">
            ...
            <button type="submit" name="login_btn" class="btn action-button w-100 py-3 mt-2">Déverrouiller</button>
        </form>
    </div>
</div>
<?php endif; ?>
...
<div class="container">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4 mt-3 position-relative header-container">
        <div class="d-none d-md-block"></div>
        
        <h1 class="header-main header-main-title m-0 flex-grow-1 text-center">Tableau de Bord "Steal A Brainrot"</h1>
        
        <div class="header-logout-div" style="min-width: 60px; text-align: right;">
            <?php if ($is_logged_in): ?>
                <a href="?logout=1" class="btn btn-sm btn-outline-danger header-logout-btn" title="Déconnexion"><i class="fas fa-power-off"></i></a>
            <?php endif; ?>
        </div>
    </div>
    
    <p class="text-center mb-4 text-muted small">
        Connecté : <strong><?= isset($_SESSION['user_name']) ? htmlspecialchars($_SESSION['user_name']) : 'Invité' ?></strong> 
        <span class="badge bg-secondary ms-1"><?= ucfirst($user_role) ?></span>
    </p>

    <?php if($user_role === 'admin'): ?>
    <div class="d-flex justify-content-center mb-4">
        <a href="add_retrait.php" class="btn btn-lg action-button shadow-sm w-100 w-md-auto" style="font-size: 1rem;">
            <i class="fas fa-plus-circle"></i> Ajouter un retrait (USD)
        </a>
    </div>
    <?php endif; ?>
    ...
    <div class="row g-2 mb-4 justify-content-center">
        <div class="col-4 col-md-3">
            <div class="badge-stat">
                ...
                <div><span class="val"><?= number_format($max_win, 0, ',', ' ') ?> $</span></div>
            </div>
        </div>
        <div class="col-4 col-md-3">
            <div class="badge-stat">
                <i class="fas fa-chart-line text-info"></i>
                <div><span class="val"><?= number_format($avg_win, 0, ',', ' ') ?> $</span></div>
            </div>
        </div>
        <div class="col-4 col-md-3">
            <div class="badge-stat">
                <i class="fas fa-bullseye text-danger"></i>
                <div><span class="val"><?= number_format($projection_usd, 0, ',', ' ') ?> $</span></div>
            </div>
        </div>
    </div>
    ...
    <div class="row g-3 mb-5">
        ...
        <div class="col-6 col-md-3">
            <div class="card p-3 text-center h-100">
                <div class="text-muted small">📈 Total (12 mois)</div>
    ...
    <div class="row g-4 mb-5">
        <div class="col-12">
            <div class="card p-4">
                <h5 class="mb-3 text-center text-md-start" style="color: #4a6fa5; font-weight: 600; font-size:1.1rem;">🏃 La Course : Ce mois vs Mois Dernier</h5>
    ...
    <div class="row g-4 mb-5">
        <div class="col-12">
            <div class="card p-4">
                <h5 class="text-center mb-3" style="font-size:1.1rem;">📈 CA Hebdomadaire (8 Semaines - USD)</h5>
    ...
    <div class="history-card mb-5">
        <h2 class="history-title">🗓️ Historique Retraits</h2>
        ...
        <div id="monthly-content-container">
            ...
            <div id="content-<?= $mois_key ?>" class="monthly-content <?= $active_class ?>">
                <div class="table-responsive">
                    <table class="table table-hover table-striped detail-table table-custom small">
    ...
    <div class="history-card mb-5">
        ...
        <div class="table-responsive">
            <table class="table table-hover table-striped table-custom small">
                <thead><tr><th>Mois</th><th>Nb</th><th>USD</th><th>EUR</th></tr></thead>
                <tbody>
                    <?php foreach ($stats_mensuelles as $stat): ?>
                    <tr>
                        <td><?= substr(traduire_mois(date('F', strtotime($stat['mois'] . '-01'))), 0, 3) . ' ' . date('y', strtotime($stat['mois'] . '-01')) ?></td>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    new Chart(ctxCumul, {
        ...
        data: {
            ...
            datasets: [
                { label: 'Ce Mois-ci', data: <?= json_encode($data_cumul_actuel) ?>, borderColor: '#4a6fa5', backgroundColor: 'rgba(74, 111, 165, 0.1)', borderWidth: 3, pointRadius: 2, fill: true, tension: 0.3 },
        ...
        options: { responsive: true, maintainAspectRatio: false, interaction: { mode: 'index', intersect: false }, plugins: { legend: { position: 'top', labels: { boxWidth: 10, font: { size: 10 } } }, tooltip: { callbacks: { label: c => c.dataset.label + ': ' + new Intl.NumberFormat('fr-FR',{style:'currency',currency:'USD'}).format(c.parsed.y) } } }, scales: { x: { grid: { display: false }, ticks: { font: { size: 10 } } }, y: { beginAtZero: true, grid: { borderDash: [2, 2] }, ticks: { font: { size: 10 } } } } }
    });
    ...
    new Chart(weeklyCtx, {
        type: 'bar',
        data: { labels: <?= json_encode($labels_hebdo) ?>, datasets: [{ label: 'USD $', data: <?= json_encode($data_hebdo) ?>, backgroundColor: 'rgba(74, 111, 165, 0.7)', borderColor: 'rgba(74, 111, 165, 1)', borderWidth: 1 }] },
        options: { responsive: true, maintainAspectRatio: false, scales: { y: { beginAtZero: true, ticks: { font: { size: 10 } } }, x: { ticks: { font: { size: 9 } } } }, plugins: { legend: { display: false }, tooltip: { callbacks: { label: c => new Intl.NumberFormat('fr-FR',{style:'currency',currency:'USD'}).format(c.parsed.y) } } } }
    });
